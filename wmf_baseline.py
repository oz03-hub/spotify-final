...
class WMFModel:
    ...
    def _build_track_tf_matrix(self):
        ...
        for track_idx, track_id in enumerate(
            tqdm(self.track_ids, desc="Building TF matrix")
        ):
    ...
    def score_query_tracks(self, query_tokens, track_ids):
        ...
        for pos, track_id in enumerate(track_ids):
            track_idx = self.track_id_to_idx.get(track_id)
            if track_idx is not None:
                valid_positions.append(pos)
                valid_indices.append(track_idx)
